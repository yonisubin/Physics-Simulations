...
m1=1
m2=1.95 # Relative mass of the second type of atom with respect to type 1
        # The size will adjust automatically
...
d_M4=0
k=0.5
M1=Circle((M1_eq+d_M1,0.5),0.05*m1,color="blue")
plt.gca().add_patch(M1)
M2=Circle((M2_eq+d_M2,0.5),0.05*m2,color="red")
plt.gca().add_patch(M2)
M0=Circle((M0_eq,0.5),0.05*m2,color="red")
plt.gca().add_patch(M0)
M3=Circle((M3_eq+d_M3,0.5),0.05*m1,color="blue")
plt.gca().add_patch(M3)
M4=Circle((M4_eq+d_M4,0.5),0.05*m2,color="red")
...
# M1.center=(0.25,0.25)
def update():
    f_M1=-(1/m1)*(k*(M1.center[0]-M2.center[0]-a)-k*(M0.center[0]-M1.center[0]-a))
    f_M2=-(1/m2)*(k*(M2.center[0]-M3.center[0]-a)-k*(M1.center[0]-M2.center[0]-a))
    f_M3=-(1/m1)*(k*(M3.center[0]-M4.center[0]-a)-k*(M2.center[0]-M3.center[0]-a))
    M1.v+=f_M1*0.01
    M2.v+=f_M2*0.01
    M3.v+=f_M3*0.01
